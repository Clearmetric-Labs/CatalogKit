# clearmetric-core validation report
...
- Raw reverse scan returned:
  ...
  - `stg_payments.amount`
- `clearmetric-core` returned the same impact set through one project-level traversal over stable `column:` IDs.
...
- dbt `child_map` for `seed.jaffle_shop.raw_payments` yields model descendants:
  ...
  - `orders`
  - `customers`
- `clearmetric-core` resolves the affected **columns** instead:
  - `stg_payments.amount`
  - `customers.customer_lifetime_value`
  - `orders.amount`
  - `orders.bank_transfer_amount`
...
- `clearmetric-core` answers the impact question in one headless command against a manifest **or a plain SQL folder**, and emits a mergeable `CatalogArtifact` rather than tool-specific intermediate files.
...
Launch `clearmetric-core` on the impact-analysis claim now that the
